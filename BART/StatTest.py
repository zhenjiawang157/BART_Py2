...
def stat_plot(stat,tfs,ID,args,col):
    ...
    plt.ylabel('Rank Score',fontsize = 12)#,fontsize=10)
    plotdir = args.outdir+os.sep+'{}_plot'.format(args.ofilename)
    try:
        os.makedirs(plotdir)
    except:
        sys.exit('Plotdir: {} could not be created.'.format(plotdir))
    ...
    by,ty = [],[]      
    for xi in x:
        by.append(sum(i< xi for i in background )/float(len(background)))
        ty.append(sum(i< xi for i in target )/float(len(target)))
...
def stat_test(AUCs,args): 
    ...
    print('Statistical tests starts.\n')
    tfs = {}
    sam1 = []
    ...
    with open(statfile,'w') as statout:
        statout.write('TF\t{}\t{}\t{}\t{}\t{}\n'.format('statistic','pvalue','zscore','max_auc','avg_rank'))
        for i in sorted(stat.index,key=lambda x: stat.loc[x]['rank_avg_z_p_a'],reverse=False):
            statout.write('{}\t{:.3f}\t{:.3e}\t{:.3f}\t{:.3f}\t{:.1f}\n'.format(i,stat.loc[i]['score'],stat.loc[i]['pvalue'],stat.loc[i]['zscore'],stat.loc[i]['max_auc'],stat.loc[i]['rank_avg_z_p_a']))
